...
(function(){
  ...
  if(cur&&window.matchMedia("(hover:hover)").matches){
    ...
    document.querySelectorAll("button,.photo-card").forEach(el=>{el.addEventListener("mouseenter",()=>cur.classList.add("big"));el.addEventListener("mouseleave",()=>cur.classList.remove("big"));});
  }
  ...
  // ── MUSIC ──
  const audio=document.getElementById("bgAudio"),overlay=document.getElementById("musicOverlay"),moBtn=document.getElementById("moBtn");
  let musicUnlocked=false;
  function startMusic(){if(!audio||musicUnlocked)return;audio.volume=0.55;audio.loop=true;audio.play().then(()=>{musicUnlocked=true;if(overlay)overlay.classList.remove("show");}).catch(()=>{if(overlay)overlay.classList.add("show");});}
  function unlockAndPlay(){if(!audio)return;musicUnlocked=true;audio.volume=0.55;audio.loop=true;audio.play().catch(()=>{});if(overlay)overlay.classList.remove("show");}
  if(moBtn)moBtn.addEventListener("click",unlockAndPlay);
  if(overlay)overlay.addEventListener("click",unlockAndPlay);

  // ── RIPPLE ──
  function addRipple(el,e){const r=document.createElement("span"),rect=el.getBoundingClientRect();const size=Math.max(rect.width,rect.height)*2;const cx=(e.clientX||e.touches?.[0]?.clientX||rect.left+rect.width/2)-rect.left-size/2;const cy=(e.clientY||e.touches?.[0]?.clientY||rect.top+rect.height/2)-rect.top-size/2;r.style.cssText=`position:absolute;width:${size}px;height:${size}px;border-radius:50%;background:rgba(255,255,255,.3);transform:scale(0);left:${cx}px;top:${cy}px;animation:ripple .6s ease-out forwards;pointer-events:none;`;el.appendChild(r);setTimeout(()=>r.remove(),600);}
  ...
  function showScreen(id){
    ...
    if(c){c.classList.add("fade-out");setTimeout(()=>{c.classList.remove("active","fade-out");n.classList.add("active","fade-in");window.scrollTo(0,0);if(id==="birthdayScreen"){startFlowers();setTimeout(startMusic,600);initBirthdayAnimations();}},380);}
    else{n.classList.add("active","fade-in");window.scrollTo(0,0);if(id==="birthdayScreen"){startFlowers();setTimeout(startMusic,600);initBirthdayAnimations();}}
  }
  ...
  document.getElementById("surpriseBtn").addEventListener("click",function(){launchConfetti();setTimeout(()=>{showScreen("birthdayScreen");initSparkles();buildHBText();startTypewriter();},320);for(let i=0;i<40;i++)setTimeout(spawnHeart,i*40);});
  ...
  // ── APPLE SCROLL PARALLAX ──
  function initAppleParallax(){
    const sections=document.querySelectorAll(".apple-section");
    const heroCard=document.querySelector(".apple-hero-card");
    function onScroll(){
      sections.forEach(sec=>{
        const rect=sec.getBoundingClientRect();
        const vh=window.innerHeight;
        const progress=1-(rect.bottom/vh);
        // parallax bg
        const bg=sec.querySelector(".apple-bg");
        if(bg){
          const shift=Math.min(Math.max(progress*18,0),18);
          bg.style.transform=`scale(1.08) translateY(${shift}px)`;
        }
        // fade in text when section is in view
        if(rect.top<vh*.75&&rect.bottom>0){
          sec.classList.add("in-view");
        }
      });
      // hero card
      if(heroCard){
        const bs=document.getElementById("birthdayScreen");
        if(bs){
          const scrolled=bs.scrollTop||window.scrollY;
          if(scrolled<50)heroCard.classList.add("visible");
        }
      }
    }
    window.addEventListener("scroll",onScroll,{passive:true});
    document.getElementById("birthdayScreen").addEventListener("scroll",onScroll,{passive:true});
    ...
    const obs=new IntersectionObserver(entries=>{
      entries.forEach(e=>{
        if(e.isIntersecting){
          ...
          // stagger photo cards in same grid
          if(e.target.classList.contains("photo-card")){
            const siblings=e.target.parentElement.querySelectorAll(".photo-card");
            siblings.forEach((s,i)=>{if(!s.classList.contains("in-view"))setTimeout(()=>s.classList.add("in-view"),i*80);});
          }
        }
      });
    },{threshold:.1,rootMargin:"0px 0px -40px 0px"});
    document.querySelectorAll(".photo-card,.grid-section-title,blockquote,.message-card").forEach(el=>obs.observe(el));
  }
  ...
  function initTimeline(){
    ...
    document.querySelectorAll(".tl-item").forEach(item=>obs.observe(item));
  }
  ...
  function initBirthdayAnimations(){
    setTimeout(()=>{
      ...
      initTimeline();
      // show hero card
      const hc=document.querySelector(".apple-hero-card");
      if(hc)setTimeout(()=>hc.classList.add("visible"),400);
    },400);
  }
  ...
  function startTypewriter(){const el=document.getElementById("messageText");if(!el||el.dataset.typed)return;el.dataset.typed="1";el.innerHTML="";const cursor=document.createElement("span");cursor.id="typewriter-cursor";el.appendChild(cursor);let i=0;const iv=setInterval(()=>{if(i>=MSG.length){clearInterval(iv);setTimeout(()=>cursor.remove(),1200);return;}cursor.insertAdjacentText("beforebegin",MSG[i]);i++;},26);}
